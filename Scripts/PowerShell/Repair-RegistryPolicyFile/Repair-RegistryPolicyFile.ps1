...
#>
    ...
    Param (
        ...
        [string]$Path = $(Join-Path -Path $env:WinDir -ChildPath 'System32\GroupPolicy\Machine\Registry.pol')
    )
    Begin {
    ...
    Process {
        ...
        Catch {
            $Result = "Could not test Registry.pol [$Path]. `n$($PSItem.Exception.Message)"
        }
        Finally {
            Write-Output $Result
        }
...
If ($TestRegistryPolicyFile -eq 'NonCompliant') {
    ...
    Catch {
        $Result = "Could not repair Registry.pol [$Path]. `n$($PSItem.Exception.Message)"
    }
    Finally {
