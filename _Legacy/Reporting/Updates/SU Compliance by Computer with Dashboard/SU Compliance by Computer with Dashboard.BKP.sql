...
.NOTES
    ...
    Release notes
        https://MEM.Zone/GIT/blob/master/Reporting/Updates/SU%20Compliance%20by%20Computer%20Classification%20and%20Severity/CHANGELOG.md
...
DECLARE @HideNotDeployed AS INT           = 1;
...
FROM fn_rbac_ClientCollectionMembers(@UserSIDs) AS CollectionMembers
    INNER JOIN fn_rbac_Update_ComplianceStatus(@UserSIDs) AS ComplianceStatus ON CollectionMembers.ResourceID = ComplianceStatus.ResourceID
        --AND ComplianceStatus.Status IN (0, 2, @Installed)       --0 Unknown, 2 Required, 3 Installed
...
WHERE CollectionMembers.CollectionID = @CollectionID
    AND IsDeployed  = @HideNotDeployed
...
SELECT
    ...
    , ComplianceStatus  =
        CASE
            ...
            ELSE ComplianceStatus
        END
    , ComplianceState  =
    /* Set Compliance by checking if there are any Required updates, or if the ComplianceStatus is NULL */
    CASE
    WHEN ComplianceStatus IS NULL THEN 'Unknown'
    WHEN SUM(CASE WHEN ComplianceStatus = 'Required' THEN 1 ELSE 0 END) OVER (Partition By ArticleID) = 0  THEN 'Compliant'
    ELSE 'Non-Compliant'
    END
    , Classification	=
    CASE
    WHEN Classification IS NULL THEN 'All Classifications'
    ELSE Classification
    END
    , Severity
    , ArticleID
